...
#값 넣기. 위처럼 tuple로 묶어줘야 들어간다.
for s in scores: #기존 Data 넣기
    ws.append(s)
...
for idx, score in enumerate(scores, start = 2): #2번째 행 부터(제목 제거) 
    sum_val = sum(score[1:]) - score[3] + 10  #총점 - 퀴즈 2를 빼주고 10점 더해서 만점 처리.
    ws.cell(row=idx, column=8).value = "=SUM(B{}:G{})".format(idx,idx) #파일은 이미 만점처리 되어 있음.
